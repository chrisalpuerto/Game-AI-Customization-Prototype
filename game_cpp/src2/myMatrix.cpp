...
float myMatrix::solve_Det_3x3() {
  return m_Det_3x3.m[0][0] * (m_Det_3x3.m[1][1] * m_Det_3x3.m[2][2] - m_Det_3x3.m[1][2] * m_Det_3x3.m[2][1]) -
...
void myMatrix::init_RotXY_3x3(float angle) {
  ...
  m_RotXY_3x3.m[0][0] = std::cos(angle);
  m_RotXY_3x3.m[1][0] = -std::sin(angle);
  m_RotXY_3x3.m[0][1] = std::sin(angle);
  m_RotXY_3x3.m[1][1] = std::cos(angle);
  m_RotXY_3x3.m[2][2] = 1.0f;
}

void myMatrix::init_RotXZ_3x3(float angle) {
  angle = angle * 3.14159f / 180.0f;
  m_RotXZ_3x3.m[0][0] = std::cos(angle);
  m_RotXZ_3x3.m[2][0] = -std::sin(angle);
  m_RotXZ_3x3.m[1][1] = 1.0f;
  m_RotXZ_3x3.m[0][2] = std::sin(angle);
  m_RotXZ_3x3.m[2][2] = std::cos(angle);
}

void myMatrix::init_RotYZ_3x3(float angle) {
  angle = angle * 3.14159f / 180.0f;
  m_RotYZ_3x3.m[0][0] = 1.0f;
  m_RotYZ_3x3.m[1][1] = std::cos(angle);
  m_RotYZ_3x3.m[2][1] = -std::sin(angle);
  m_RotYZ_3x3.m[1][2] = std::sin(angle);
  m_RotYZ_3x3.m[2][2] = std::cos(angle);
}
...
void myMatrix::init_RotXY_4x4(float angle) {
  angle = angle * 3.14159f / 180.0f;
  m_RotXY_4x4.m[0][0] = std::cos(angle);
  m_RotXY_4x4.m[1][0] = -std::sin(angle);
  m_RotXY_4x4.m[0][1] = std::sin(angle);
  m_RotXY_4x4.m[1][1] = std::cos(angle);
...
void myMatrix::init_RotXZ_4x4(float angle) {
  angle = angle * 3.14159f / 180.0f;
  m_RotXY_4x4.m[0][0] = std::cos(angle);
  m_RotXY_4x4.m[2][0] = std::sin(angle);
  m_RotXY_4x4.m[1][1] = 1.0f;
  m_RotXY_4x4.m[0][2] = -std::sin(angle);
  m_RotXY_4x4.m[2][2] = std::cos(angle);
  m_RotXY_4x4.m[3][3] = 1.0f;
}
...
void myMatrix::init_RotYZ_4x4(float angle) {
  angle = angle * 3.14159f / 180.0f;
  m_RotYZ_4x4.m[0][0] = 1;
  m_RotYZ_4x4.m[1][1] = std::cos(angle);
  m_RotYZ_4x4.m[2][1] = -std::sin(angle);
  m_RotYZ_4x4.m[1][2] = std::sin(angle);
  m_RotYZ_4x4.m[2][2] = std::cos(angle);
...
void myMatrix::Translate_4x4(myPoint* in, myPoint* out) {
  myPoint d = *in;
  out->tuple[0] = d.tuple[0] * m_Translate_4x4.m[0][0] + d.tuple[1] * m_Translate_4x4.m[1][0] + d.tuple[2] * m_Translate_4x4.m[2][0] + m_Translate_4x4.m[3][0];
  out->tuple[1] = d.tuple[0] * m_Translate_4x4.m[0][1] + d.tuple[1] * m_Translate_4x4.m[1][1] + d.tuple[2] * m_Translate_4x4.m[2][1] + m_Translate_4x4.m[3][1];
  out->tuple[2] = d.tuple[0] * m_Translate_4x4.m[0][2] + d.tuple[1] * m_Translate_4x4.m[1][2] + d.tuple[2] * m_Translate_4x4.m[2][2] + m_Translate_4x4.m[3][2];
}
...
void myMatrix::Scale_4x4(myPoint* in, myPoint* out) {
  myPoint d = *in;
  out->tuple[0] = d.tuple[0] * m_Scale_4x4.m[0][0] + d.tuple[1] * m_Scale_4x4.m[1][0] + d.tuple[2] * m_Scale_4x4.m[2][0] + m_Scale_4x4.m[3][0];
  out->tuple[1] = d.tuple[0] * m_Scale_4x4.m[0][1] + d.tuple[1] * m_Scale_4x4.m[1][1] + d.tuple[2] * m_Scale_4x4.m[2][1] + m_Scale_4x4.m[3][1];
  out->tuple[2] = d.tuple[0] * m_Scale_4x4.m[0][2] + d.tuple[1] * m_Scale_4x4.m[1][2] + d.tuple[2] * m_Scale_4x4.m[2][2] + m_Scale_4x4.m[3][2];
}

void myMatrix::Rotate_XY_4x4(Vector* in, Vector* out) {
  Vector d = *in;
  out->tuple[0] = d.tuple[0] * m_RotXY_4x4.m[0][0] + d.tuple[1] * m_RotXY_4x4.m[1][0] + d.tuple[2] * m_RotXY_4x4.m[2][0] + m_RotXY_4x4.m[3][0];
  out->tuple[1] = d.tuple[0] * m_RotXY_4x4.m[0][1] + d.tuple[1] * m_RotXY_4x4.m[1][1] + d.tuple[2] * m_RotXY_4x4.m[2][1] + m_RotXY_4x4.m[3][1];
  out->tuple[2] = d.tuple[0] * m_RotXY_4x4.m[0][2] + d.tuple[1] * m_RotXY_4x4.m[1][2] + d.tuple[2] * m_RotXY_4x4.m[2][2] + m_RotXY_4x4.m[3][2];
}

void myMatrix::Rotate_XZ_4x4(Vector* in, Vector* out) {
  Vector d = *in;
  out->tuple[0] = d.tuple[0] * m_RotXZ_4x4.m[0][0] + d.tuple[1] * m_RotXZ_4x4.m[1][0] + d.tuple[2] * m_RotXZ_4x4.m[2][0] + m_RotXZ_4x4.m[3][0];
  out->tuple[1] = d.tuple[0] * m_RotXZ_4x4.m[0][1] + d.tuple[1] * m_RotXZ_4x4.m[1][1] + d.tuple[2] * m_RotXZ_4x4.m[2][1] + m_RotXZ_4x4.m[3][1];
  out->tuple[2] = d.tuple[0] * m_RotXZ_4x4.m[0][2] + d.tuple[1] * m_RotXZ_4x4.m[1][2] + d.tuple[2] * m_RotXZ_4x4.m[2][2] + m_RotXZ_4x4.m[3][2];
}

void myMatrix::Rotate_YZ_4x4(Vector* in, Vector* out) {
  Vector d = *in;
  out->tuple[0] = d.tuple[0] * m_RotYZ_4x4.m[0][0] + d.tuple[1] * m_RotYZ_4x4.m[1][0] + d.tuple[2] * m_RotYZ_4x4.m[2][0] + m_RotYZ_4x4.m[3][0];
  out->tuple[1] = d.tuple[0] * m_RotYZ_4x4.m[0][1] + d.tuple[1] * m_RotYZ_4x4.m[1][1] + d.tuple[2] * m_RotYZ_4x4.m[2][1] + m_RotYZ_4x4.m[3][1];
  out->tuple[2] = d.tuple[0] * m_RotYZ_4x4.m[0][2] + d.tuple[1] * m_RotYZ_4x4.m[1][2] + d.tuple[2] * m_RotYZ_4x4.m[2][2] + m_RotYZ_4x4.m[3][2];
}
